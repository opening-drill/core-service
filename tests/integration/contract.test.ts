...
describe.skipIf(!hasTestDb)('Live-data contract (DB)', () => {
  ...
  beforeEach(async () => {
    ...
    auth = basicHeader(rbac.adminUsername, rbac.adminPassword);
  });
  ...
  it('runs the report-target flow with contract shapes', async () => {
    ...
    // image ref for AI
    const ref = await request(app).get(`/api/storage/pictures/${pictureId}`).set('Authorization', auth);
    ...
    const gotTarget = await request(app).get(`/api/targets/${targetId}`).set('Authorization', auth);
    ...
    const detail = await request(app).get(`/api/events/${eventId}`).set('Authorization', auth);
    ...
    const list = await request(app).get('/api/events').set('Authorization', auth);
    ...
    const ctx = await request(app).get(`/api/events/${eventId}/ai-context`).set('Authorization', auth);
  ...
  it('serves aircraft list, live, path, track, batch and patch', async () => {
    ...
    const listFree = await request(app).get('/api/aircraft?status=free').set('Authorization', auth);
  ...
  it('lists active polygons with the contract shape', async () => {
    ...
    const res = await request(app).get('/api/polygons?active=true').set('Authorization', auth);
  ...
  it.skipIf(!hasS3)('uploads a picture in one multipart request', async () => {
